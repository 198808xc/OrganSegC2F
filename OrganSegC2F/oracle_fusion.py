...
training_margin = int(sys.argv[5])
testing_margin = int(sys.argv[6])
organ_ID = int(sys.argv[7])
learning_rate = float(sys.argv[8])
gamma = float(sys.argv[9])
snapshot_path = os.path.join(snapshot_path, 'fine:' + sys.argv[8] + ',' + str(training_margin))
result_path = os.path.join(result_path, 'oracle:' + sys.argv[8] + ',' + str(training_margin))
starting_iterations = int(sys.argv[10])
...
timestamp = {}
timestamp['X'] = sys.argv[14]
timestamp['Y'] = sys.argv[15]
timestamp['Z'] = sys.argv[16]
...
result_directory_ = {}
for plane in ['X', 'Y', 'Z']:
    result_name = result_name_from_timestamp(result_path, current_fold, \
        plane, 'F', slice_thickness, organ_ID, iteration, volume_list, timestamp[plane])
    if result_name == '':
