...
snapshot_path = os.path.join(snapshot_path, 'coarse:' + sys.argv[11])
starting_iterations = int(sys.argv[13])
step = int(sys.argv[14])
max_iterations = [int(s) for s in sys.argv[15].split(',')]
timestamp = sys.argv[16]
snapshot_name = 'FD' + str(current_fold) + ':' + \
    plane + 'C' + str(slice_thickness) + '_' + str(organ_ID) + '_' + timestamp
...
if __name__ == '__main__':
    ...
    coarse_surgery.interp(solver.net, interp_layers)
